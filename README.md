...
- **Table view** – Sortable, filterable table with toggleable columns. Default columns: Date, Match, Club, Discipline, Registration, Participants, SSI Link.
...
When a match doesn't include coordinates, the fetch script looks up the organizer in `data/organizer-geocache.json`. If not cached, it calls Nominatim (OpenStreetMap) with the club name and country code.

**Manual overrides** – pin any organizer to a fixed location by adding an entry to `data/manual-coords.json`. This file takes priority over both the geocache and the API coordinates:
...
data/manual-coords.json         highest-priority coordinate overrides
data/organizer-geocache.json    Nominatim geocoding cache
